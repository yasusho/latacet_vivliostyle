 ...
 * @returns Properties.
 */
const createProperties = (attributes) => {
    if (!attributes) {
        return {};
    }
    const props = {};
 ...
 * @returns AST of `<body>`.
 */
const createBody = (metadata, tree) => {
    // <body>...</body>
    const contents = tree.type === 'root' && Array.isArray(tree.children)
 ...
 * @returns AST of `<html>`.
 */
const createHTML = (metadata, tree, vfile) => {
 ...
 * @returns Transformer.
 */
export const mdast = (data) => (tree, vfile) => {
    return {
        type: 'root',
        children: [
