 ...
 * @param parent `<p>` tag.
 */
const wrapFigureImg = (img, parent) => {
    if (!(img.properties && parent.properties)) {
        return;
    }
    parent.tagName = 'figure';
    parent.children.push(h('figcaption', { 'aria-hidden': 'true' }, propertyToString(img.properties.alt)));
};
    ...
    visit(tree, 'element', (node, index, parent) => {
        ...
        if (is(node, 'pre') &&
            ...
            if (Array.isArray(maybeCode.properties.className)) {
                parent.children[index] = h('figure', { class: maybeCode.properties.className[0] }, h('figcaption', propertyToString(maybeTitle)), node);
            }
            return;
        }
